...
def train_model(model, train_loader, val_loader, criterion, optimizer, scheduler=None, num_epochs=100, patience=10, output_dir=None, device=None):
    ...
    参数:
        ...
        num_epochs (int, optional): 训练的最大周期数。默认值为 100。
        patience (int, optional): 早停的耐心值。默认值为 10。
        output_dir (str, optional): 输出目录，用于保存模型和图表。
    ...
    for epoch in range(num_epochs):
        ...
        for inputs, labels in tqdm(train_loader, desc=f'Epoch {epoch + 1}/{num_epochs}', leave=False):
            ...
            # ******** 添加梯度裁剪 (重要) ********
            torch.nn.utils.clip_grad_norm_(model.parameters(), max_norm=1.0)  # 限制梯度范数为 1.0
