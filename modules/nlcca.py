...
class NCA(nn.Module):
    ...
    def forward(self, x):
        ...
        zy = y.permute(0, 2, 1)  # [b, 1, c]
        zg = torch.matmul(zy, y)  # [b, 1, 1]
        batch = zg.shape[0]
        v = zg.squeeze(-1).permute(1, 0).expand((self.feature_channel, batch))
        v = v.unsqueeze_(-1).permute(1, 2, 0)  # (b, 1, c)

        atten = self.conv(y.transpose(1, 2))  # (b, 1, c)
        atten = atten + v
        atten = self.conv_end(atten)  # (b, 1, c)
        atten = atten.transpose(1, 2)  # (b, c, 1)

        atten_score = self.soft(atten)  # (b, c, 1)
        return x * atten_score  # (b, c, len)
